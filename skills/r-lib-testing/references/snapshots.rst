...
Snapshot tests record expected output in human-readable files rather
than inline code. They are ideal for:
...
``tests/testthat/_snaps/{test-file}/{test-name}.md`` containing the
captured output.
...
Common uses: - Remove timestamps or session IDs - Normalize file paths -
Strip API keys or tokens - Remove stochastic elements
...
Variants save to ``_snaps/{variant}/{test}.md`` instead of
``_snaps/{test}.md``.
...
- **Fail on new snapshots in CI** - testthat 3.3.0+ does this
  automatically
...
Each snapshot includes: - Test name as heading - Code that generated the
