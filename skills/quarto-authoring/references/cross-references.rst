...
Quarto provides a unified cross-reference system for figures, tables,
equations, sections, theorems, and more.

Label Prefix System
-------------------

All cross-referenceable elements require a label starting with a type
prefix:
...
For consistency, prefer using div syntax for cross-referenceable
elements:
...
Both syntaxes work, but div syntax provides a more consistent pattern
across all element types (figures, tables, theorems, etc.).
...
Available theorem types: ``thm``, ``lem``, ``cor``, ``prp``, ``cnj``,
``def``, ``exm``, ``exr``.
...
- `Quarto
  Cross-References <https://quarto.org/docs/authoring/cross-references.html>`__
- `Figures <https://quarto.org/docs/authoring/figures.html>`__
- `Tables <https://quarto.org/docs/authoring/tables.html>`__
