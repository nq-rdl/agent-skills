...
Use ``digraph`` for directed graphs, ``graph`` for undirected. Standard
DOT features (subgraphs, node styling, rank direction) all work.
...
Use ``%%| fig-width`` and ``%%| fig-height`` cell options. Diagrams are
responsive by default in HTML; disable with
...
When using ``mermaid-format: js`` (the default for HTML), Quarto
controls theming and may override custom theme configurations. The YAML
config block inside the Mermaid cell might appear to have no effect.
...
With ``svg`` or ``png`` format, the YAML config block inside Mermaid
cells will be respected.
...
- `Quarto Diagrams <https://quarto.org/docs/authoring/diagrams.html>`__
- `Mermaid Documentation <https://mermaid.js.org/>`__
- `Graphviz Documentation <https://graphviz.org/documentation/>`__
- `DOT Language <https://graphviz.org/doc/info/lang.html>`__
