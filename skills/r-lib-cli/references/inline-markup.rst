...
2.  `Basic Syntax <#basic-syntax>`__
3.  `Code & Syntax Classes <#code--syntax-classes>`__
4.  `Files & Paths Classes <#files--paths-classes>`__
5.  `Communication Classes <#communication-classes>`__
6.  `Values & Data Classes <#values--data-classes>`__
7.  `Emphasis Classes <#emphasis-classes>`__
8.  `Documentation Classes <#documentation-classes>`__
9.  `Special Classes <#special-classes>`__
10. `Vector Collapsing <#vector-collapsing>`__
11. `Advanced Patterns <#advanced-patterns>`__
12. `Pluralization <#pluralization>`__
13. `Performance Considerations <#performance-considerations>`__
14. `Quick Reference Table <#quick-reference-table>`__
...
Inline markup classes format text within cli messages using
``{.class content}`` syntax. They integrate with glue string
interpolation and work across all cli functions: ``cli_text()``,
``cli_abort()``, ``cli_warn()``, ``cli_inform()``, ``cli_alert_*()``,
etc.
...
- Classes can be nested (see `Advanced Patterns <#advanced-patterns>`__)
...
**When to use:** - Multi-token code expressions - Syntax examples -
Configuration snippets - When other code classes are too specific
...
**Automatic formatting:** - Adds ``()`` suffix automatically - Supports
``package::function`` notation - Both ``.fn`` and ``.fun`` are
equivalent

**When to use:** - Referring to functions by name - Suggesting which
function to call - Error messages about function usage
...
**When to use:** - Parameter validation errors - Documenting function
arguments - Explaining argument behavior
...
**When to use:** - Type checking errors - Documenting expected types -
...
**When to use:** - Base R types: integer, double, character, logical -
Generic type categories - Type coercion messages
...
**Automatic features:** - Adds “a”/“an” article automatically - Uses
friendly names (“data frame” not “data.frame”) - Handles pluralization
for vectors
...
**Best practices:** - Use for any file reference - Works with relative
and absolute paths - Handles home directory expansion
...
**When to use .file vs .path:** - ``.file`` - Files, scripts, documents
...
**Features:** - Creates ``mailto:`` links in supported terminals -
...
**Features:** - Creates clickable hyperlinks in supported terminals -
...
**Syntax:** - Format: ``{.href [text](url)}`` - Markdown-style link
syntax - Text and URL can be styled differently

**Link types:** - HTTP/HTTPS: ``https://example.com`` - Help topics:
``help:topic`` - Vignettes: ``vignette:package::topic``
...
**Automatic formatting:** - Quoted for character values - Unquoted for
numeric values - Handles vectors with collapsing
...
**When to use:** - Variable names in R code - Column names in data
frames - Field names in objects
...
**When to use:** - System environment variables - R-specific environment
variables - Configuration via environment
...
**When to use:** - Named list elements - Data frame columns (prefer
``.var`` for analysis context) - S4 slots - Object attributes
...
**When to use:** - Literal string values - Pattern strings - Format
strings - When ``.val`` formatting is too generic
...
**Rendering:** - Typically italic or colored - Lighter emphasis than
``.strong``
...
**Rendering:** - Typically bold or brightly colored - Stronger emphasis
than ``.emph``
...
**Features:** - Creates link to help topic in RStudio - Supports
``package::topic`` notation - Fallback to plain text in terminals
...
**When to use:** - Internal documentation sections - Vignette sections -
...
**Features:** - Links to package vignettes - Format:
``package::vignette-name``
...
**Features:** - Creates executable link in RStudio - Click to run code
in console - Formatted as code in terminals
...
**Rendering:** - Typically in keyboard key style - May show as boxed or
distinct formatting
...
**Rendering:** - Formats as ” or ” with appropriate styling - Maintains
class formatting for surrounding elements
...
**Features:** - Distinct package name formatting - May include
CRAN/GitHub links in supported environments
...
**When to use:** - Definition list terms - Glossary entries - Key-value
pair keys
...
**When to use:** - Definition list descriptions - Glossary definitions -
...
**Rules:** - Length 1: no separators - Length 2: ” and ” separator -
...
**Nesting guidelines:** - Outer class determines primary formatting -
Inner classes add semantic meaning - Keep nesting depth reasonable (2-3
levels max)
...
**Common patterns:** - ``{?y/ies}`` - directory/directories - ``{?/s}``
- item/items - ``{?is/are}`` - is/are - ``{?/es}`` - box/boxes -
...
**When to use qty():** - When quantity appears elsewhere - Complex
expressions with multiple numbers - Explicit pluralization control
...
+------------------------+------------------+--------------------------------+---------------------+
| ``.dt``                | Definition term  | ``{.dt term}``                 | term (in definition |
|                        |                  |                                | list)               |
+------------------------+------------------+--------------------------------+---------------------+
| ``.dd``                | Definition desc  | ``{.dd description}``          | description (in     |
|                        |                  |                                | definition list)    |
+------------------------+------------------+--------------------------------+---------------------+
