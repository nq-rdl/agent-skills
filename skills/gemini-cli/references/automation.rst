...
Shell-level patterns for automating Gemini CLI in scripts, CI pipelines,
and batch workflows. These complement the MCP tools (which handle the
common cases) for direct CLI use.
...
Use ``GEMINI_SYSTEM_MD`` to set a system prompt — store prompts in
``.gemini/prompts/``:
