...
`Cmd` is `type Cmd func() Msg` — an async I/O function.
`View` is a struct (see below) — NOT a `string`.
...
**Note:** `tea.WithAltScreen()` does NOT exist in v2. Set `v.AltScreen = true` in your `View()` method instead.

---

## View Type

`View()` returns `tea.View`, not `string`. Create with `tea.NewView`:
