...
> **All imports:** `charm.land/bubbles/v2/<name>`
> Do NOT invent methods. Use ONLY the signatures listed here.
...
| `Validate` | `func(string) error` | Real-time validation |
...
**Constructor:** `list.New(items []Item, delegate ItemDelegate, width, height int) Model`
...
Options: `spinner.WithSpinner(s Spinner)`, `spinner.WithStyle(style lipgloss.Style)`
...
Used internally by `textinput` and `textarea`. Direct use is uncommon but available.
