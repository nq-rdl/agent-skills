...
Styles are **immutable** — every method returns a new Style. Store the
result.
...
Lipgloss v2 moved color downsampling to print time. Use these instead of
``fmt.Println(style.Render(text))`` for correct color handling across
...
These functions detect the terminal’s color profile and downsample
true-color to 256/16/no-color as needed.
