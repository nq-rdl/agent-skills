...
This document defines the role scaffolding patterns used by the team.
The key principle is **lean structure** — only create what you need, and
grow from there.

Role Tiers
----------

Roles naturally fall into complexity tiers. Start at the tier that
matches the role’s actual needs. Don’t scaffold Tier 3 for a Tier 1 job.

Tier 1 — Task-Only Role
~~~~~~~~~~~~~~~~~~~~~~~

For roles that just execute tasks with no configuration surface.
Validation roles, preflight checks, one-shot operations.
...
For roles that install packages, template configuration, and manage
services. This is the most common pattern.
...
**defaults/main.yml** — Every variable the role uses that a consumer
might want to change. Include a comment block at the top explaining the
role’s purpose:
...
For complex roles that also need static files, custom variables
(non-overridable), or metadata for dependencies. Rarely needed.
...
**When to use ``vars/`` vs ``defaults/``**: - ``defaults/`` — values the
consumer should tune (ports, package lists, feature flags) - ``vars/`` —
internal constants the consumer should not change (OS-specific paths,
computed values)

**When to use ``meta/``**: - Only when the role depends on other roles
via Galaxy-style dependencies - If the role is self-contained, skip
``meta/`` entirely
...
Always starts with ``---``. Delegates to concern files rather than
containing logic directly (unless the role is Tier 1 with only a few
tasks).
...
When a task file should only run under certain conditions, apply the
condition at the import level in ``main.yml``, not inside the task file:
...
3. **Write ``defaults/main.yml`` first** (if Tier 2+) — this defines the
   role’s API
