...
The `opencode.json` file lives at the project root (or `~/.config/opencode/opencode.json` for global config). Multiple config files merge with precedence: remote → global → custom → project → inline.
...
| `default_agent` | string | Agent to use by default (e.g., `"build"`) |
| `autoupdate` | boolean | Auto-update OpenCode on startup |
| `instructions` | string[] | Paths/globs to rule files to include as context |
...
Agent fields: `description`, `mode`, `model`, `temperature`, `top_p`, `steps`, `tools`, `permission`, `prompt`, `color`, `hidden`, `disable`
...
Glob patterns are supported. Files are read relative to the project root.
...
Values: `"auto"` (default), `"prune"` (remove old messages), `"reserved"` (keep all)
