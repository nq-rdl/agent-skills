...
Plugins bundle tools and subscribe to lifecycle hooks. They execute server-side in OpenCode's Bun runtime.
...
- Keys matching a hook name → hook handler function
- Key `"tool"` → map of tool name → tool definition
...
| `$` | Bun shell | `$\`command\`` for shell execution |
| `directory` | `string` | Absolute path to working directory |
| `worktree` | `string` | Absolute path to git worktree root |
...
|-------|------|-----------|
| `tool.execute.before` | Before any tool runs | `(input: { tool, args }, output: { args }) => void` |
| `tool.execute.after` | After any tool runs | `(input: { tool, args, result }) => void` |
...
| Event | When | Signature |
|-------|------|-----------|
| `shell.env` | Before shell commands execute | `(input, output: { env }) => void` |
...
| Event | When | Signature |
|-------|------|-----------|
| `session.created` | New session started | `(input: { id }) => void` |
| `session.updated` | Session metadata changed | `(input: { id }) => void` |
| `session.compacted` | Context was compacted | `(input: { id }) => void` |
| `session.deleted` | Session removed | `(input: { id }) => void` |
| `session.idle` | Session finished | `(input: { id }) => void` |
| `session.error` | Session errored | `(input: { id, error }) => void` |
| `session.status` | Status message emitted | `(input: { id, status }) => void` |
| `session.diff` | File changes applied | `(input: { id, diff }) => void` |
...
Declare dependencies that your plugin needs in `.opencode/package.json`:
...
OpenCode runs `bun install` at startup and makes these available to plugin imports.

## Distribution

**Local plugin** — commit `.opencode/plugins/` to your project repo. Team members get it automatically.

**npm package** — publish as a normal npm package:
1. Export the plugin function as the default or named export
2. Users install: `bun add my-plugin`
...
Test plugin tools directly by extracting them and calling with a mock context:
