...
Custom tools give the LLM a new callable function. The LLM decides when
to call them based on the ``description``.
...
| Location                               | Scope                            |
+========================================+==================================+
| ``.opencode/tools/<name>.ts``          | Project-local (current project   |
|                                        | only)                            |
+----------------------------------------+----------------------------------+
...
+---------------+--------------------------------------+-------------------------------+
| ``git.ts``    | ``export const status = tool(...)``, | ``git_status``, ``git_log``   |
|               | ``export const log = tool(...)``     |                               |
+---------------+--------------------------------------+-------------------------------+

Custom tools with the same name as a built-in tool **override** the
built-in.
...
Return strings, objects, or arrays — OpenCode serialises them for the
LLM.
...
OpenCode runs ``bun install`` at startup. Import normally in your tool
file.
