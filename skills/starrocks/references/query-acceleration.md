...
Condensed from the [Query Acceleration](https://docs.starrocks.io/docs/category/query-acceleration/)
docs.
...
|-----------|--------|--------|----------|
| CBO statistics | Low | High | Always — run after bulk loads |
| Sync materialized view | Low | High | Single-table aggregates |
| Async materialized view | Medium | High | Multi-table joins + aggregates |
| Colocate join | Medium | High | Frequent joins between large tables |
| Bloom filter index | Low | Medium | High-cardinality equality filters |
| Query cache | Low | Medium | Repeated identical queries (dashboards) |
| Flat JSON | Low | Medium | Frequent JSON field extraction |
| JIT compilation | Low | Low–Medium | Complex expressions |
...
- When query plans look suboptimal (unexpected full scans or bad join orders)

## Materialized Views

### Synchronous (Single-Table)

Auto-refreshed on data load. Transparent query rewrite — the optimizer automatically
uses the MV when it matches the query pattern.
...
- No joins
- Limited to simple aggregations (SUM, COUNT, MIN, MAX, HLL_UNION, BITMAP_UNION)
- Cannot include WHERE clause

### Asynchronous (Multi-Table)

Scheduled or manual refresh. Supports joins, complex aggregations, and query rewrite.
...
**Refresh strategies:**
- `REFRESH ASYNC EVERY (INTERVAL ...)` — periodic
- `REFRESH MANUAL` — on-demand via `REFRESH MATERIALIZED VIEW mv_name`
- `REFRESH ASYNC` — auto-refresh when base table changes
...
Eliminates network shuffle by ensuring joined tables are physically co-located.
...
Automatically extracts frequently accessed JSON fields into columnar storage for
faster access (v3.3+):
...
Useful when JSON columns contain semi-structured data with stable top-level fields.
...
Best for queries with complex CASE/WHEN, arithmetic, or string expressions.

## Sorted Streaming Aggregate

When data is sorted by the GROUP BY columns (matching the sort key), StarRocks
can use a streaming aggregate instead of a hash aggregate — using less memory
and running faster:
...
Works automatically when GROUP BY columns match the table's sort key prefix.
...
7. **Join strategy**: colocate, broadcast, or shuffle? Is colocate possible?
...
| Anti-Pattern | Fix |
|-------------|-----|
| No CBO stats collected | Run `ANALYZE TABLE` after loads |
| MV not being used in query rewrite | Check column alignment; run `EXPLAIN` |
| Colocate join not triggered | Verify same bucket count, column types, and group |
| Query cache on frequently changing data | Disable cache for real-time tables |
| JSON field access without flat JSON | Enable flat JSON for stable top-level fields |
| GROUP BY on non-sort-key columns | Consider sort key redesign or add a sync MV |
