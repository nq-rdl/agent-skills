...
Worked examples showing how the orchestrator drives TDD cycles. All examples use `claude:subagent` as the default backend. For other backends, use the `dispatch` skill to determine routing.
...
Use `claude:agent-team` for the red phase to dispatch multiple features simultaneously.
...
If you have `jules`, `gemini-cli`, or `pi-rpc` installed, load the `dispatch` skill to determine
how to route phases to external backends. The dispatch skill handles session lifecycle — you just
pass the uniform phase input format and receive the status token.
